...
<template>
  <section class="panel trace-panel">
    <div class="panel-heading">
      ...
      <span class="panel-caption">实时轨迹</span>
    </div>
    <div v-if="!steps.length && !running" class="trace-empty">
      <div class="empty-icon"><FileSearch :size="22" /></div>
    ...
    <div v-else class="trace-list">
      <div v-if="running" class="trace-item active-trace">
        ...
        <div>
          <strong>模型正在收集证据</strong>
          <small>请求 DevAtlas 与服务状态工具</small>
        </div>
        <span class="trace-time">LIVE</span>
      </div>
      <div
        ...
        class="trace-item"
      >
        <span class="trace-marker" :class="step.status === 'success' ? 'success' : 'failure'">
          <Check v-if="step.status === 'success'" :size="13" />
          <X v-else :size="13" />
        </span>
        <div>
          <strong>{{ nodeLabel(step.node) }}</strong>
          <small>{{ step.tool_name ? toolLabel(step.tool_name) : step.action }}</small>
        </div>
        <span class="trace-time">R{{ step.iteration }}</span>
      </div>
