...
function submit() {
  ...
  fieldError.value = ''
  emit('submit', { title: title.value.trim(), content: content.value.trim(), knowledge_base_id: selected, top_k: topK.value })
}
</script>
<template>
  <section class="panel intake-panel"><div class="panel-heading"><div><span class="panel-index">01</span><h3>Incident brief</h3></div><span class="panel-caption">输入现场信息</span></div>
    <form class="incident-form" @submit.prevent="submit"><label>故障标题<input v-model="title" maxlength="200" required /></label><label>日志 / 现象<textarea v-model="content" maxlength="20000" rows="9" required></textarea></label>
      <div class="field-row"><label>知识库<div class="number-input"><BookOpen :size="15" /><select v-model.number="knowledgeBaseId" :disabled="loadingKnowledgeBases || !knowledgeBases.length" required><option :value="null" disabled>{{ knowledgeBasePlaceholder }}</option><option v-for="option in knowledgeBases" :key="option.id" :value="option.id">{{ option.name }}（#{{ option.id }}）</option></select><ChevronDown :size="14" /></div></label><label>召回数量<div class="number-input"><Search :size="15" /><select v-model.number="topK"><option v-for="value in [3, 5, 8, 10]" :key="value" :value="value">Top {{ value }}</option></select><ChevronDown :size="14" /></div></label></div>
      <p v-if="knowledgeBaseError" class="form-error field-note">{{ knowledgeBaseError }}<button type="button" class="field-action" @click="emit('reloadKnowledgeBases')">重新加载</button></p>
      <p v-else-if="!loadingKnowledgeBases && !knowledgeBases.length" class="form-error field-note">当前账号没有可用知识库，请先在 DevAtlas 创建。</p>
