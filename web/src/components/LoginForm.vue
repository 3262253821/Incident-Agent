...
<template>
  <div class="auth-panel">
    ...
    <form @submit.prevent="emit('submit', username, password)">
      ...
      <label>密码<input v-model="password" type="password" autocomplete="current-password" required placeholder="输入密码" /></label>
      <p v-if="error" class="form-error"><AlertTriangle :size="15" /> {{ error }}</p>
      <button class="primary-button full" type="submit" :disabled="loading"><LoaderCircle v-if="loading" class="spin" :size="17" /><ArrowRight v-else :size="17" />{{ loading ? '验证中' : '登录并进入' }}</button>
    </form>
