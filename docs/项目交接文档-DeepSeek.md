...
>
> **最后更新**：2026-09-15（P1-3 整组完成 + P1-6-1 知识库下拉选择）
>
> **提交锚点**：上一轮代码提交范围 `f5af0d7` … `d0d711f`（`d0d711f` = P1-3-4）；本轮新增 **P1-6-1**（知识库下拉选择 + Agent 只读代理端点），其提交号见 `docs/项目补充优化.md` 第 12.6 节。**当前 HEAD 请以 `git log --oneline -5` 为准**（本文档自身也是提交之一，写死哈希会立刻过时）。
>
...
| 指标 | 最初 | 现在 |
| --- | --- | --- |
| 提交数 | 0 | **30 个提交**（`f5af0d7` … `d0d711f` + P1-6-1，另有若干文档提交） |
| 测试用例 | 18 passed | **315 passed, 0 warnings** |
| 测试文件 | 4 个（用户原有） | **25 个**（另有 `tests/conftest.py` 做代理变量隔离） |
| ruff（全仓） | 193 个错误 | **0** |
| 弃用/SQLAlchemy 警告 | 131 条 | **0** |
| 数据库迁移 | 1 个 | 3 个（新增 `370ee3c8987d` 中断标记列、`c78153d58823` 降级摘要列） |
| 清单完成度 | — | **已勾 29 项（含审计基线）/ 未勾 19 项** |

**唯一的任务源是 `docs/项目补充优化.md`**（勾选表 + 完成记录 + 执行顺序记录）。本文只做导航与背景。

---

## 4. 已完成内容一览（29 项，每项都满足"实现 + 测试 + 推送"）
...
- **B（下一步）**：P1-4-1 → P1-4-5（顺便人工确认 CI 是否绿）。
...
- **pytest**：会让 23 条测试在任何断言之前失败 → 已由 `tests/conftest.py` 在会话开始时清掉代理变量修好，现在应当是 **315 passed**；
...
- **测试不需要 MySQL**：全部 315 条测试使用 SQLite 内存库，可离线运行；只有 `alembic check` 需要本机 MySQL。
...
> 读 `docs/项目交接文档-DeepSeek.md` 和 `docs/项目补充优化.md`，用户已定顺序 **A → B → C**，A（P1-6-1 知识库下拉选择）已完成，现在做 **B 的第一步：P1-4-1 补齐设计文档要求的测试**。按约定：一次一项、实现+测试+推送齐了才打勾、不跳项、不用 `git add .`，每项都要做反证并写完成记录。

（B 的第二步是 P1-4-5 确认 CI：workflow 已推送，但**必须人工打开** <https://github.com/3262253821/Incident-Agent/actions> 看到绿色才能勾选；GitHub API 查询一直被 403 拒绝。）
