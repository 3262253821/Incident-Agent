...
| 指标 | 最初 | 现在 |
| --- | --- | --- |
| 提交数 | 0 | **31 个提交**（`f5af0d7` … `d0d711f` + P1-6-1 + P1-4-1，另有若干文档提交） |
...
| P1-4-1 | 补齐设计文档要求的测试：新增 3 个测试文件 45 条（输入边界、统一工具结果协议、§17.2 案例 2/3/5/6/7/9 + 大输入）；修掉"**纯空白标题/内容能进模型**"的真实缺口（`str_strip_whitespace`）；25 个既有测试文件补 `设计文档章节：…`，新增 `docs/测试对照-设计文档章节.md` 做十条案例逐条对照 | 见 12.6 |
