...
>
> **最后更新**：2026-09-15（**P1-5-5 与 P1-5-6 一起完成：历史筛选/重跑/失败细分 + 422 字段级提示**）。当前进度：**C 项（前端质量）六步全部完成**（P1-5-1 ~ P1-5-6），清单里只剩 **P2 七项产品增强**与"明确不做"四项。前端测试 50 → 97 → **137 条**。**下一步 = P2-1（SSE 流式分析）或用户指定的其他 P2 项**。准确数字请跑 §12.1 的三条命令。
>
> **提交锚点**：每一步的提交号都写在 `docs/项目补充优化.md` 的完成记录与 3.3 节里；**准确 HEAD 请直接跑 `git log --oneline -5`（工作区应当是干净的）**，写死哈希会立刻过时。
>
> **文档地图（新会话先看这张表）**：
>
> | 文档 | 作用 |
...
>
> **下一步要做什么**：C 项（前端质量 P1-5-1 ~ P1-5-6）**六步全部完成**（P1-5-5 与 P1-5-6 按用户指示一起做，见 `docs/项目补充优化.md` 3.3 节"顺序调整记录"）；**当前只剩 P2 七项产品增强**（P2-1 SSE 流式分析 / P2-2 部分成功继续报告 / P2-3 固定评测集 / P2-4 Token 与成本观测 / P2-5 真实服务状态 provider / P2-6 Playwright E2E / P2-7 Docker 部署复现）。P2 的顺序用户尚未指定，**等指示**；未完成项与各自缺口见 `docs/项目补充优化.md` 2.2 节。
>
...
| CI | 无 | **绿**（最新：P1-5-4 的 run `34947266183`，两个 job 全过，前端 job 已含"运行单测"一步） |
...
| P1-5-4 | 统一 401 处理并区分网络故障：`web/src/api/client.ts` 加响应拦截器，判据从"错误文案 `includes('401')`"改为 **HTTP 状态码 + 后端统一契约的 `error_code`**（修掉了"文案变成中文 detail 后永远不会登出"的静默失效）；`restore()` 区分"401/403 清会话"与"连不上/超时/上游 5xx 只提示、保留 Token"；跳登录页时带上 `redirect`（并用 `safeRedirect` 挡住开放重定向）；前端测试 50 → **97 条**（含真实 axios 实例与真实 store 的链路测试） | `4ad3492`（记录 `c07cba2`） |
