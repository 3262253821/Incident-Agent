...
from tools import run_tool as run_l3_tool  # noqa: E402
...
def build_graph():
    ...
    # observe节点，负责处理 ToolMessage
    def observe_tools_node(
        state: AgentState,
    ) -> dict[str, object]:
        """读取 ToolMessage，保存工具事实和执行轨迹。"""
        ...
        # 找出新增的ToolMessage，即未处理的ToolMessage
        ...
        for message in new_tool_messages:
            ...
            # 获取工具名，如果ToolMessage有name属性，就用name属性，否则用unknown_tool
            tool_name = getattr(message, "name", None) or "unknown_tool"

            # 保存工具结果到observations列表
            observations.append(
                {
        ...
        print(
            f"本轮新增工具结果：{len(new_tool_messages)} 条"
        )
    ...
    def degrade_node(
        ...
        error = state["error"] or "工具执行失败或没有有效结果"
    ...
    def route_after_agent(
        ...
        # 取出messages中的最后一个消息，假设第一轮agent刚结束，这时messages中只有一个消息，即AIMessage
    ...
    # ToolNode 只负责执行工具和产生 ToolMessage，此时还没有把结果写入 observations
...
def run_case(
    ...
    # 执行图，从START节点开始，即从build_graph函数中定义的START节点开始执行
